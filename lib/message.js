...
var ECDSA = ravencore.crypto.ECDSA;
var Signature = ravencore.crypto.Signature;
var sha256sha256 = ravencore.crypto.Hash.sha256sha256;
...
Message.MAGIC_BYTES = new Buffer('Ravencoin Signed Message:\n');
...
/**
 * Will sign a message with a given ravencoin private key.
 *
...
/**
 * Will return a boolean of the signature is valid for a given ravencoin address.
 * If it isn't the specific reason is accessible via the "error" member.
 *
 * @param {Address|String} ravencoinAddress - A ravencoin address
 * @param {String} signatureString - A base64 encoded compact signature
 * @returns {Boolean}
 */
Message.prototype.verify = function verify(ravencoinAddress, signatureString) {
  $.checkArgument(ravencoinAddress);
  $.checkArgument(signatureString && _.isString(signatureString));

  if (_.isString(ravencoinAddress)) {
    ravencoinAddress = Address.fromString(ravencoinAddress);
  }
  ...
  var signatureAddress = Address.fromPublicKey(publicKey, ravencoinAddress.network);

  // check that the recovered address and specified address match
  if (ravencoinAddress.toString() !== signatureAddress.toString()) {
